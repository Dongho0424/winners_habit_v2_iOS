...
class HabitDetailVC: UIViewController, FSCalendarDelegate, FSCalendarDataSource, FSCalendarDelegateAppearance {
    ...
    lazy var habitColor = hexToUIColor(hex: habitVO.habit.color)
    ...
    func initHabitDetails() {
        ...
        self.habitImg.image = self.habitVO.habitImg
        self.habitTitle.text = self.habitVO.habit.habitName
        ...
        case "min":
            self.attribute.text = "0/\(habitVO.habit.defaultAttributeValue!) min"
        case "pages":
            self.attribute.text = "0/\(habitVO.habit.defaultAttributeValue!) 장"
        default:
            ()
        }
        ...
        if self.habitVO.habit.alarmFlag {
            self.alarmTimeSwitch.isOn = true
            self.alarmTime.text = convertAlarmTime(time: self.habitVO.habit.alarmTime!)
            self.alarmTime2.text = convertAlarmTime(time: self.habitVO.habit.alarmTime!)
    ...
    func calendar(_ calendar: FSCalendar, cellFor date: Date, at position: FSCalendarMonthPosition) -> FSCalendarCell {
        ...
        print("dongho \(dateStringDetail(date: date))")
        return cell
    }
