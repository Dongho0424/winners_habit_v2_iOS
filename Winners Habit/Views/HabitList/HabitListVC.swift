...
class HabitListVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private let viewModel = HabitListVM()
    ...
    func getHabitIconImg(_ index: Int) -> UIImage? {

        var habitVO = self.habits[index]

        if let savedImg = habitVO.iconImage {
            return savedImg
        } else {
            if let url = URL(string: habitVO.icon),
               let imgData = try? Data(contentsOf: url) {
                habitVO.iconImage = UIImage(data: imgData)
                return habitVO.iconImage
            } else {
                return UIImage()
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.habitTitle.text = habitVO.habitName

        DispatchQueue.main.async {
            cell.habitImg.image = self.getHabitIconImg(indexPath.row)
        }

        if habitVO.alarmFlag {
            cell.habitAlarmTime.text = convertAlarmTime(time: habitVO.alarmTime!)
...
//            cell.alarmImg = nil
        ...
        case "min":
            cell.habitAttr.text = "0/\(habitVO.defaultAttributeValue!) min"
        case "pages":
            cell.habitAttr.text = "0/\(habitVO.defaultAttributeValue!) 장"
        default:
            ()
        }
